...
const STRIPE_RATES: Record<string, { pct: number; fixed: number; label: string; flag: string; tag: string }> = {
...
type Fees = { amount: number; safeSendFee: number; stripeFee: number; buyerPays: number; sellerReceives: number; net: number };

function calcFees(amount: number, cardType: string): Fees {
  ...
  return { amount, safeSendFee: r(safeSendFee), stripeFee: r(stripeFee), buyerPays: r(buyerPays), sellerReceives: r(amount), net: r(net) };
}

function nzd(n: number) {
  return new Intl.NumberFormat("en-NZ", { style: "currency", currency: "NZD", minimumFractionDigits: 2 }).format(Math.abs(n));
}
...
function Pill({ children, bg, col }: { children: React.ReactNode; bg: string; col: string }) {
  return (
    <span style={{ display: "inline-block", padding: "2px 9px", borderRadius: 99, fontSize: 11, fontWeight: 700, background: bg, color: col }}>
      {children}
    </span>
  );
}
...
const STAGES = [
  {
    id: "created", icon: "📋", label: "Deal Created", actor: "Seller", actorCol: TEAL,
    desc: "Seller fills in item details and buyer email. System emails and SMSs the buyer a secure payment link.",
    deadline: "Buyer has 7 days to pay", urgent: false,
    missed: "Deal auto-cancels after 7 days. No money was ever involved.",
    money: "No money involved yet", moneyCol: SLATE,
    impl: "Hourly job: cancel where state='created' AND now > pay_by_deadline", built: true,
  },
  {
    id: "funded", icon: "💳", label: "Buyer Pays", actor: "Buyer", actorCol: BLUE,
    desc: "Payment captured by Stripe. Funds held in escrow — seller cannot access them.",
    deadline: "Seller has 5 business days to ship", urgent: true,
    missed: "Automatic full refund to buyer after 5 business days. No action needed from buyer.",
    money: "Held in escrow — fully protected", moneyCol: BLUE,
    impl: "Hourly job: refund where state='funded' AND now > ship_by_deadline", built: true,
  },
  {
    id: "shipped", icon: "📦", label: "Seller Ships", actor: "Seller", actorCol: TEAL,
    desc: "Seller enters tracking number. TrackingMore registers and monitors for courier scans.",
    deadline: "First courier scan within 48 hours", urgent: true,
    missed: "No scan within 48h = shipment flagged. Buyer can cancel and receive a full refund.",
    money: "Still held in escrow", moneyCol: BLUE,
    impl: "Job every 6h: flag where state='shipped' AND now > shipped_at + 48h with no scan", built: true,
  },
  {
    id: "delivered", icon: "✅", label: "Delivered", actor: "Courier", actorCol: "#7c3aed",
    desc: "Courier confirms delivery. Buyer gets SMS + email: confirm receipt or raise dispute within 48 hours.",
    deadline: "Buyer has 48 hours to act", urgent: true,
    missed: "No action within 48h = funds auto-release to seller. Dispute window closes.",
    money: "Auto-releases to seller in 48h if no action", moneyCol: AMBER,
    impl: "Hourly job: release where state='delivered' AND now > delivered_at + 48h", built: true,
  },
  {
    id: "disputed", icon: "⚠️", label: "Dispute Raised", actor: "Buyer / Seller", actorCol: RED,
    desc: "Funds frozen immediately. Admin reviews courier data, messages, and evidence from both parties.",
    deadline: "Admin resolves within 14 calendar days", urgent: true,
    missed: "If unresolved after 14 days, buyer receives a full automatic refund.",
    money: "Frozen — pending admin decision", moneyCol: RED,
    impl: "Hourly job: refund where state='disputed' AND now > dispute_resolve_by", built: true,
  },
  {
    id: "complete", icon: "🎉", label: "Funds Released", actor: "SafeSend", actorCol: TEAL,
    desc: "Stripe Connect transfer fires immediately. Seller bank receives funds in 1-2 business days.",
    deadline: "Bank transfer: 1-2 business days", urgent: false,
    missed: "N/A — this is the successful end state.",
    money: "Transferred to seller", moneyCol: TEAL,
    impl: "stripe.transfers.create() with idempotency key prevents any double-payment", built: true,
  },
];
...
const WORST_CASES = [
  { lbl: "Buyer never pays",                 cur: "7 days → auto-cancel",            rec: "Built ✓", done: true  },
  { lbl: "Seller never ships after payment", cur: "5 business days → auto-refund",   rec: "Built ✓", done: true  },
  { lbl: "No courier scan within 48h",       cur: "48h → buyer can cancel",           rec: "Built ✓", done: true  },
  { lbl: "Buyer ignores delivery scan",      cur: "48h → auto-release",              rec: "Built ✓", done: true  },
  { lbl: "Dispute runs unresolved",          cur: "14 days → auto-refund buyer",     rec: "Built ✓", done: true  },
];
...
const DISPUTE_STEPS = [
  ...
  { day: "Day 5",  label: "Resolution target",     col: AMBER, desc: "Admin issues a binding decision within 5 business days." },
  ...
  return (
    <Layout>
      <div style={{ fontFamily: SANS, background: "#f0f4f8", minHeight: "100vh", color: "#1a2332" }}>
        ...
        {/* Hero */}
        <div style={{ background: `linear-gradient(135deg, ${TEAL_DARK} 0%, ${TEAL} 55%, #14b8a6 100%)`, padding: "44px 24px 60px", textAlign: "center" }}>
          <div style={{ display: "inline-block", background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 99, padding: "4px 16px", fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "white", marginBottom: 16 }}>
            Transparent pricing — no hidden fees
          </div>
          <h1 style={{ fontSize: 36, fontWeight: 800, color: "white", marginBottom: 10, lineHeight: 1.15 }}>
            Exactly what you pay.<br />Exactly what you get.
          </h1>
          <p style={{ color: "rgba(255,255,255,0.82)", fontSize: 15, maxWidth: 480, margin: "0 auto", lineHeight: 1.6 }}>
            Every fee, every deadline, every protection — shown upfront. No surprises when real money is involved.
          </p>
        </div>

        {/* Tabs */}
        <div style={{ background: "white", borderBottom: "1px solid #e2e8f0", display: "flex", justifyContent: "center", overflowX: "auto" }}>
          {tabs.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              style={{ padding: "15px 24px", fontSize: 14, fontWeight: 600, border: "none", background: "none", cursor: "pointer", borderBottom: `3px solid ${tab === t.id ? TEAL : "transparent"}`, color: tab === t.id ? TEAL : SLATE, transition: "all 0.15s", whiteSpace: "nowrap" }}
            >
              {t.label}
            </button>
          ))}
        ...
        <div style={{ maxWidth: 940, margin: "0 auto", padding: "28px 18px 60px" }}>
          ...
          {tab === "calc" && (
            <div>
              ...
              <div style={{ background: "white", borderRadius: 18, padding: "24px 26px", boxShadow: "0 4px 20px rgba(0,0,0,0.07)", marginBottom: 18 }}>
                    ...
                    <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                      {Object.entries(STRIPE_RATES).map(([k, v]) => (
                        <button
                          ...
                          style={{ padding: "7px 13px", borderRadius: 9, border: `2px solid ${cardType === k ? TEAL : "#e2e8f0"}`, background: cardType === k ? TEAL_LITE : "white", color: cardType === k ? TEAL : SLATE, fontWeight: 700, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}
                        >
                          {v.flag} {v.label}
                          {v.tag && <span style={{ background: TEAL, color: "white", borderRadius: 99, padding: "1px 6px", fontSize: 10, fontWeight: 700 }}>{v.tag}</span>}
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                <input
                  type="range" min={20} max={2500} value={amount} step={5}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#94a3b8", marginBottom: 14 }}>
                  <span>$20 minimum (slider)</span><span>$2,500 maximum</span>
                </div>
                ...
                <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                  {[100, 250, 500, 850, 1200, 2000].map((v) => (
                    <button
                      key={v}
                      onClick={() => setAmount(v)}
                      style={{ padding: "5px 12px", borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: "pointer", border: `1.5px solid ${amount === v ? TEAL : "#e2e8f0"}`, background: amount === v ? TEAL_LITE : "#f8fafc", color: amount === v ? TEAL : SLATE }}
                    >
                      ${v}
                    </button>
                  ))}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 18 }}>
                ...
                <div style={{ background: "white", borderRadius: 18, overflow: "hidden", boxShadow: "0 4px 20px rgba(0,0,0,0.07)" }}>
                  ...
                  <div>
                    {[
                      { label: "Item price", val: fees.amount, col: "#1e293b" },
                      { label: "SafeSend fee (4%, min $5)", val: fees.safeSendFee, col: TEAL },
                      { label: `Stripe processing (${stripePct}% + $0.30)`, val: fees.stripeFee, col: BLUE },
                    ].map((row) => (
                      <div key={row.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 20px", borderBottom: "1px solid #f1f5f9" }}>
                        <span style={{ fontSize: 13, color: "#475569" }}>{row.label}</span>
                        <span style={{ fontWeight: 700, color: row.col, fontFamily: MONO }}><AnimNum val={row.val} /></span>
                      </div>
                    ))}
                ...
                <div style={{ background: "white", borderRadius: 18, overflow: "hidden", boxShadow: "0 4px 20px rgba(0,0,0,0.07)" }}>
                  ...
                  <div style={{ padding: "24px 20px", textAlign: "center" }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: SLATE, textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: 8 }}>You receive exactly</div>
                    <div style={{ fontSize: 44, fontWeight: 800, color: TEAL, lineHeight: 1, fontFamily: MONO }}><AnimNum val={fees.sellerReceives} /></div>
                  ...
                  <div style={{ margin: "0 18px 18px", background: TEAL_LITE, borderRadius: 10, padding: "12px 14px", fontSize: 12, color: "#166534", lineHeight: 1.65 }}>
                    Fees are paid by the buyer, not deducted from you.<br />
              ...
              <div style={{ background: "white", borderRadius: 18, padding: "20px 24px", boxShadow: "0 4px 20px rgba(0,0,0,0.07)", marginBottom: 18 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap" }}>
                  {[
                    { label: "Platform fee collected", val: fees.safeSendFee, col: TEAL, prefix: "+" },
                    { label: "Stripe fee (SafeSend pays)", val: fees.stripeFee, col: RED, prefix: "-" },
                    { label: "Net revenue", val: fees.net, col: fees.net >= 0 ? TEAL : RED, prefix: fees.net >= 0 ? "" : "-" },
                  ].map((row, i) => (
                    <div key={i} style={{ flex: 1, minWidth: 140, padding: "10px 16px", borderRight: i < 2 ? "1px solid #f1f5f9" : "none" }}>
                      <div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 4, fontWeight: 600 }}>{row.label}</div>
                      <div style={{ fontSize: i === 2 ? 22 : 18, fontWeight: 800, color: row.col, fontFamily: MONO }}>{row.prefix}{nzd(row.val)}</div>
                    </div>
                  ))}
                </div>
                {fees.net < 2 && (
                  <div style={{ marginTop: 12, background: RED_LITE, border: "1px solid #fecaca", borderRadius: 8, padding: "10px 14px", fontSize: 12, color: "#991b1b" }}>
                    Margin is very tight here. The $5 minimum fee protects against losses on domestic cards — but international cards on small deals can produce negative net revenue.
                  </div>
                )}
              </div>
                    ...
                    <thead>
                      <tr style={{ background: "#f8fafc" }}>
                        {["Sale price", "SafeSend fee", "Stripe fee", "Buyer pays", "Seller gets", "SafeSend net"].map((h) => (
                          <th key={h} style={{ padding: "10px 16px", textAlign: "left", fontWeight: 700, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.05em", color: SLATE, borderBottom: "2px solid #e2e8f0" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {TABLE_AMOUNTS.map((a) => {
                        ...
                        return (
                          <tr key={a} onClick={() => setAmount(a)} style={{ background: active ? TEAL_LITE : "white", cursor: "pointer" }}>
          ...
          {tab === "timeline" && (
            <div>
              <div style={{ background: TEAL_LITE, border: "1px solid #86efac", borderRadius: 10, padding: "13px 16px", marginBottom: 20, fontSize: 13, color: "#14532d", lineHeight: 1.6 }}>
                <strong>All deadlines run in server code — not Stripe dashboard.</strong> Stripe's dashboard is monitoring only. All refunds and cancellations are triggered programmatically by scheduled server jobs.
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
                {STAGES.map((stage, i) => (
                  <div
                    ...
                    style={{ background: "white", borderRadius: 16, overflow: "hidden", boxShadow: openStage === stage.id ? "0 6px 28px rgba(15,118,110,0.14)" : "0 2px 10px rgba(0,0,0,0.05)", border: `2px solid ${openStage === stage.id ? TEAL : "transparent"}`, cursor: "pointer", transition: "all 0.2s" }}
                  >
                    <div style={{ padding: "17px 20px", display: "flex", alignItems: "center", gap: 14 }}>
                      ...
                      <div style={{ textAlign: "right", flexShrink: 0 }}>
                        <div style={{ fontSize: 11, fontWeight: 700, padding: "4px 10px", borderRadius: 8, background: stage.urgent ? AMBER_LITE : TEAL_LITE, color: stage.urgent ? "#92400e" : "#14532d", marginBottom: 6 }}>
                          {stage.deadline}
                        </div>
              ...
              <div style={{ background: "white", borderRadius: 18, overflow: "hidden", boxShadow: "0 4px 20px rgba(0,0,0,0.07)" }}>
                <div style={{ padding: "16px 22px", borderBottom: "1px solid #f1f5f9", fontWeight: 700, fontSize: 14 }}>
                  Maximum time money can be held — all scenarios enforced
                </div>
                {WORST_CASES.map((row, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 20px", borderBottom: i < WORST_CASES.length - 1 ? "1px solid #f1f5f9" : "none", flexWrap: "wrap" }}>
                    <span style={{ fontSize: 13, flex: 1, minWidth: 180 }}>{row.lbl}</span>
                    <span style={{ fontSize: 12, fontFamily: MONO, color: TEAL, fontWeight: 600, background: TEAL_LITE, padding: "3px 10px", borderRadius: 6 }}>{row.cur}</span>
                    <Pill bg="#dcfce7" col="#15803d">{row.rec}</Pill>
                  </div>
                ))}
              </div>
            </div>
          ...
          {tab === "dispute" && (
            <div>
              <div style={{ background: TEAL_LITE, border: "1px solid #86efac", borderRadius: 10, padding: "13px 16px", marginBottom: 20, fontSize: 13, color: "#14532d", lineHeight: 1.6 }}>
                <strong>14-day hard cap is now enforced in server code.</strong> Once a dispute is raised, the <code>dispute_resolve_by</code> deadline is set. An hourly job auto-refunds the buyer if no admin decision is reached within 14 days.
              </div>

              {/* SLA timeline */}
              <div style={{ background: "white", borderRadius: 18, padding: "28px 24px", boxShadow: "0 4px 20px rgba(0,0,0,0.07)", marginBottom: 18 }}>
                <div style={{ fontSize: 15, fontWeight: 800, marginBottom: 28 }}>Dispute SLA</div>
                <div style={{ display: "flex", justifyContent: "space-between", position: "relative" }}>
                  ...
                  {DISPUTE_STEPS.map((s, i) => (
                    <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
                      <div style={{ width: 44, height: 44, borderRadius: "50%", background: s.col, display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: 800, fontSize: 11, marginBottom: 10, position: "relative", zIndex: 1, fontFamily: MONO }}>
                        {s.day}
                      </div>
              ...
              <div style={{ background: "white", borderRadius: 16, padding: "20px 24px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
                <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 12 }}>Terms of Service — Section 6 (current)</div>
                <div style={{ background: "#1e293b", borderRadius: 10, padding: "16px 20px", fontSize: 13, color: "#e2e8f0", lineHeight: 1.85, fontFamily: MONO }}>
                  ...
                  <div>Either party may raise a dispute while a deal is in funded, shipped, or delivered state. Raising a dispute freezes all funds immediately.</div>
                  <div style={{ marginTop: 10 }}>SafeSend will acknowledge the dispute within 1 business day and issue a binding resolution within <span style={{ color: "#86efac" }}>5 business days</span>.</div>
                  <div style={{ marginTop: 10 }}>If SafeSend cannot reach a decision within <span style={{ color: "#f87171" }}>14 calendar days</span>, the buyer automatically receives a full refund. SafeSend decisions are final. Evidence must be submitted within 48 hours.</div>
                </div>
